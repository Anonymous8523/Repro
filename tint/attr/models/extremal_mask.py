...
class ExtremalMaskNN(nn.Module):
    ...
    def forward(
        ...
        *additional_forward_args,
        ...
        # Mask data according to the formula in Enguehard 2023 https://arxiv.org/abs/2305.18840
        mask = mask[:, : x.shape[1], ...]
        x = x * mask + baselines * (1.0 - mask)  # Phi(x,m) = m * x + (1-m) * nn(x)

        # Return f(perturbed x), equivalent in the paper to f(Phi(x,m)), wher f is the classifier
        return _run_forward(
...
class ExtremalMaskNet(Net):
    ...
    def step(self, batch, batch_idx, stage):
        """
        preservation mode:
            L = lambda_1 * ||m||_1 + lambda_2 * ||NN(x)||_1 + l(f(x), f(Phi(x, m)))

        deletion mode:
            L = lambda_1 * ||1-m||_1 + lambda_2 * ||NN(x)-x||_2^2 + l(f(0), f(Phi(x, m)))
        """
        # x is the data to be perturbed
        # y is the same data without perturbation
        x, y, baselines, target, *additional_forward_args = batch

        # If additional_forward_args is only one None, set it to None
        if additional_forward_args == [None]:
            additional_forward_args = None

        # Get perturbed output: f(Phi(x,m)) in the paper
