...
from tint.datasets import Weather
...
def main(
    ...
    # Load data
    weather = Weather(n_folds=5, fold=fold, seed=seed)
    ...
    with lock:
        x_train = weather.preprocess(split="train")["x"].to(device)
        x_test = weather.preprocess(split="test")["x"].to(device)
        y_test = weather.preprocess(split="test")["y"].to(device)
