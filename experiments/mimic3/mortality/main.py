...
def main(
    ...
    if "extremal_mask_preservation" in explainers:
        ...
        perturbed_signals["extremal_mask_preservation"] = mask.net.perturbed_singal.to(device)
    ...
    if "extremal_mask_deletion" in explainers:
        ...
        attr["extremal_mask_deletion"] = _attr.to(device)  # the returned mask by deletion is flipped
    ...
    with open('results/mimic3_saliency.npz', 'wb') as fp:
        attr_numpy = {k: v.detach().numpy() for k, v in attr.items()}
        pickle.dump(attr_numpy, fp)
    with open("results/mimic3_perturbed_signals.npz", "wb") as fp:
        perturbed_signals_numpy = {k: v.detach().numpy() for k, v in perturbed_signals.items()}
        pickle.dump(obj=perturbed_signals_numpy, file=fp)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mimic3_classifier/version_3/checkpoints/epoch=99-step=17800.ckpt",
